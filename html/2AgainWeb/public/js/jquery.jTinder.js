...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		favorite: function(){
			panes.eq(current_pane).animate({"transform": "translate(" + (0) + "px," + (1080) + "px) rotate(0deg)"}, $that.settings.animationSpeed, function () {
				if($that.settings.onFavorite) {
					$that.settings.onFavorite(panes.eq(current_pane));
				}
				$that.next();
			});
		},
			...
			switch (ev.type) {
				...
				case 'touchmove':
					...
					if(touchStart === true) {
						...
						if (posX >= 30 && posY >= 5) {
							...
							panes.eq(current_pane).find($that.settings.favoriteSelector).css('opacity', 0);
							panes.eq(current_pane).find($that.settings.superLikeSelector).css('opacity', 0);
						} else if (posX < -30 && posY >= 5) {
							...
							panes.eq(current_pane).find($that.settings.favoriteSelector).css('opacity', opa);
							panes.eq(current_pane).find($that.settings.superLikeSelector).css('opacity', 0);
							panes.eq(current_pane).find($that.settings.likeSelector).css('opacity', 0);
							panes.eq(current_pane).find($that.settings.dislikeSelector).css('opacity', 0);
						} else if (posY < -30) {
							panes.eq(current_pane).find($that.settings.superLikeSelector).css('opacity', opa);
							panes.eq(current_pane).find($that.settings.favoriteSelector).css('opacity', 0);
							panes.eq(current_pane).find($that.settings.likeSelector).css('opacity', 0);
							panes.eq(current_pane).find($that.settings.dislikeSelector).css('opacity', 0);
						}
				...
				case 'touchend':
					...
					if (opa >= 1) {
						...
						// move up or down
						if (posY > 50) {
							panes.eq(current_pane).animate({"transform": "translate(" + (0) + "px," + (posY + 1080) + "px) rotate(0deg)"}, $that.settings.animationSpeed, function () {
								if($that.settings.onFavorite) {
									$that.settings.onFavorite(panes.eq(current_pane));
								}
								$that.next();
							...
							panes.eq(current_pane).find($that.settings.favoriteSelector).animate({"opacity": 0}, $that.settings.animationRevertSpeed);
